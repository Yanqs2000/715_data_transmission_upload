...
{ 
    ...
    char buf[MAXLINE - 20]; // 用于数据data的组成
    memset(buf, 'A', sizeof(buf)); //使用 memset 将所有字节设置为 'A'
    ...
    ping_data.pack_cnt = htonl(cnt);       // TP分包计数，假设为1
    ping_data.pack_sum = htonl(100);     // TP总包数，假设为1
    ...
    // 填充 buffer 的后1024字节:开头部分填充为字母 'a'，数量等于 pack_cnt 的值，剩余部分用字母 'A' 填充
    ...
    memcpy(ping_data.data, buf, sizeof(buf));
