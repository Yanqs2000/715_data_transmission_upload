...
// 生成文件名（每个线程都有独立的顺序文件）
void get_filename_from_sequence(char *filename, size_t len, const char *dir, int port) 
{
    char sequence_file[256];
    snprintf(sequence_file, sizeof(sequence_file), "%s/sequence_%d.txt", dir, port);
    int sequence_number = 1;  // 默认顺序号从 1 开始

    // 尝试打开 sequence_<port>.txt 文件以读取上次保存的顺序号
    FILE *file = fopen(sequence_file, "r");
    if (file != NULL) 
    {
    ...
    // 生成文件名，例如 <dir>/data_<port>_<sequence_number>.bin
    snprintf(filename, len, "%s/data_%d_%d.bin", dir, port, sequence_number);

    // 顺序号加 1
    sequence_number++;

    // 将新的顺序号写回 sequence_<port>.txt 以备下次使用
    file = fopen(sequence_file, "w");
    if (file != NULL) 
    {
    ...
    {
        perror("Error opening sequence file for writing");
    }
}
...
void *receive_data(void *arg) 
{
    ...
    get_filename_from_sequence(filename, sizeof(filename), directory, port);
