...
int main(int argc, char **argv)
{
    ...
    // 配置串口
    int serial_fd = USB_init(serial_device);
    if (serial_fd == -1) 
    {
        return -1;
    }
    ...
    // 检查文件夹是否存在
    if (access(nas_folder_path, F_OK) == 0) 
    {
        // F_OK 表示检查文件是否存在
    ...
    // 向串口发送数据包
    ssize_t bytes_written_start = write(serial_fd, initial_command_buffer, sizeof(initial_command_buffer));
    ...
    // 向串口发送关闭数据包
    ssize_t bytes_written_end = write(serial_fd, end_uart6_packet, sizeof(end_uart6_packet));
