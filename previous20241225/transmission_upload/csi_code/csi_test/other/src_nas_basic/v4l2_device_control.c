...
V4l2Device *v4l2_device_open(const char *dev_name,
    ...
    v4l2_device->field = V4L2_FIELD_NONE;//逐行扫描模式
    v4l2_device->fps = fps;//要捕获的帧数
    v4l2_device->pAttr->buffer_num = 3;//缓冲区数量，应该可以改(一般不超过5)
    //检测像素格式是否支持
    bool found = false;
    int i;
...
bool v4l2_device_setup(V4l2Device *v4l2_device) {
    ...
    struct v4l2_capability cap;//视频设备的功能，对应命令VIDIOC_QUERYCAP,获取设备的硬件信息
    struct v4l2_format fmt;//帧的格式，对应命令VIDIOC_G_FMT、VIDIOC_S_FMT等
    struct v4l2_streamparm stream_parm;//流类型，通常是 V4L2_BUF_TYPE_VIDEO_CAPTURE（用于视频捕捉）
                                       //或 V4L2_BUF_TYPE_VIDEO_OUTPUT（用于视频输出）
    struct v4l2_input inp;//视频输入信息，对应命令VIDIOC_ENUMINPUT

    if (ioctl(v4l2_device->pAttr->fd, VIDIOC_QUERYCAP, &cap) != 0) {//发送命令
        perror("VIDIOC_QUERYCAP");
        return false;
    }

    if (!(cap.capabilities & V4L2_CAP_VIDEO_CAPTURE)) {//表示这是一个视频捕捉设备,支持视频捕捉
    ...
    /* Set input type */
    inp.index = 0;
    inp.type = V4L2_INPUT_TYPE_CAMERA;//输入类型为摄像头
    if (ioctl(v4l2_device->pAttr->fd, VIDIOC_S_INPUT, &inp) == -1) {//设置视频输入设备，但需要重新设置视频设备的参数
        perror("VIDIOC_S_INPUT");
        return false;
    }

    CLEAR(fmt);
    fmt.type = V4L2_BUF_TYPE_VIDEO_CAPTURE;//设置帧的类型为视频捕获设备的缓冲区
    ...
    fmt.fmt.pix.bytesperline = v4l2_device->width * 2;//可能不对，不符合格式？？？
    fmt.fmt.pix.sizeimage    = v4l2_device->height * v4l2_device->width * 2;

    /* It is possible to set a failure even if it returns 0 */
    if (ioctl(v4l2_device->pAttr->fd, VIDIOC_S_FMT, &fmt) < 0) {//设置图像格式
        perror("VIDIOC_S_FMT");
        return false;
    }

    if (ioctl(v4l2_device->pAttr->fd, VIDIOC_G_FMT, &fmt) < 0) {//获取当前设备的图像格式
        perror("VIDIOC_G_FMT");
        return false;
    }
    ...
    stream_parm.parm.capture.timeperframe.numerator = 1;//设置每帧时间间隔的分子为1
    stream_parm.parm.capture.timeperframe.denominator = v4l2_device->fps;//设置每帧时间间隔的分母为帧数
