...
/* USER CODE BEGIN 2 */
void SetLed(int idx, int sta)
{
	switch(idx)
	{
		case 1:
			HAL_GPIO_WritePin(L1_GPIO_Port, L1_Pin, sta ? GPIO_PIN_RESET : GPIO_PIN_SET);
			break;
		default:
			break;
	}
...
{
	uint8_t key = 0;
	key |= (HAL_GPIO_ReadPin(K1_GPIO_Port, K1_Pin) == GPIO_PIN_RESET) ? 0x01 : 0x00;
	return key;
}
