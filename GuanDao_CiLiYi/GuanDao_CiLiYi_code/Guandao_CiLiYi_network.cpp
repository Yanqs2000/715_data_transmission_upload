...
void udp_listener_thread(const char* output_file, const char* output_file_nas)
{
    ...
    server_addr.sin_port = htons(8080);  // Port for receiving the start command
    ...
    while (true) 
    {
        ssize_t n = recvfrom(udp_socket, initial_command_buffer, sizeof(buffer), 0, (struct sockaddr*)&client_addr, &len);
        if (n > 0) 
        {
            ...
            if (initial_command_buffer[0] == 0xF0 && initial_command_buffer[1] == 0x00 
                ...
                recv_len == 32)  // 确保数据包大小为32字节
                {
                    ...
                    // 将接收到的起始命令写入文件
                    initial_command.write(initial_command_buffer, 32);


                    outfile.open(output_file, ios::binary | ios::app);
                    outfile.write(initial_command_buffer, 32);
                    if (params.if_nas == true)
                    {
                        outfile_nas.open(output_file_nas, ios::binary | ios::app);
                        outfile_nas.write(initial_command_buffer, 32);
                    }
