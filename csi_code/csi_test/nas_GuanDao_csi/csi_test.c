...
void* receive_guandao_data(void* arg) 
{
    while (!g_quit) 
    {
...
int main(int argc, char **argv)
{
    ...
    serial_fd_GuanDao = USB_init_GuanDao(serial_device_GuanDao);
    ...
    else if (access(nas_folder_path, F_OK) == -1 && params.if_nas == true)
    {
        ...
        {
            printf("nas directory is empty.\n");
            printf("maybe a new nas, need a file folder to check, please create a simple file folder in nas by hand.\n");
            printf("maybe mounting nas is failed.\n");
            params.if_nas = false;
        } 
        else if (if_nas_exist_path_result == 0) 
        {
            printf("nas directory is not empty.\n");
    ...
    if(params.if_GuanDao)
    {
        if (pthread_create(&guandao_thread, NULL, receive_guandao_data, NULL)) 
        {
            perror("pthread create");
            return -1;
        }
    ...
    while (!g_quit) 
    {
        ...
        if (v4l2_device_get_buffer(camera, 3)) //调用 v4l2_device_get_buffer 函数获取一帧数据。
        {
            ...
            if (params.if_GuanDao == true) {
                ...
                {
                    sem_wait(sem);
                    memcpy(str, date_time, SHM_SIZE); // 使用 memcpy 复制数据
                    sem_post(sem);
                }
